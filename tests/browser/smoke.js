...
async function testXrView(browser, errs) {
  ...
  const g = await page.evaluate(() => { const d = simCtl.xr.gltf(); return { v: d.asset.version, n: d.nodes.length, total: simCtl.xr.scene.nodes.length, stations: simCtl.xr.scene.nodes.filter(n => n.kind === 'station').length, buf: d.buffers[0].uri.startsWith('data:application/octet-stream;base64,'), score: /\"s\":/.test(JSON.stringify(d)) }; });
  check('xr: glTF 2.0 export carries every scene node, an embedded buffer, and no score', g.v === '2.0' && g.n === g.total && g.total >= g.stations + 3 && g.buf && !g.score, JSON.stringify(g));
  const r5 = await page.evaluate(() => { const signs = simCtl.xr.scene.nodes.filter(n => n.kind === 'sign').map(n => n.id); const bytes = new TextEncoder().encode(JSON.stringify(simCtl.xr.gltf())).buffer; const room = simCtl.xr.loadRoom(bytes); return { signs, meshes: room && room.meshes, tri: room && room.triangles, status: document.querySelector('.cxxr-status').textContent, json: !!document.querySelector('.cxxr-json'), input: !!document.querySelector('.cxxr-room') }; });
  check('xr: the room carries the honesty signs — the law, simulated here, practised live, under eighteen', r5.signs.includes('sign') && r5.signs.includes('sign-simulated') && r5.signs.includes('sign-live') && r5.signs.includes('sign-youth'), JSON.stringify(r5.signs));
  check('xr: a hall\'s own room loads from glTF bytes offline, floor set to zero, and the panel says it stays on the device', r5.meshes > 0 && r5.tri === r5.meshes * 12 && /stays on this device/.test(r5.status) && r5.json && r5.input, JSON.stringify(r5));
  const r6 = await page.evaluate(() => { const r = simCtl.xr.loadRoom(new TextEncoder().encode('{"asset":{"version":"2.0"},"buffers":[{"byteLength":4,"uri":"x.bin"}]}').buffer); return { r, status: document.querySelector('.cxxr-status').textContent }; });
  check('xr: a file with an external buffer is refused with the reason (this page fetches nothing)', r6.r === null && /fetches nothing/.test(r6.status));
