...
UNBANDED_FILLED_PACKS = {"Future-Work", "Civic & Leadership", "Language, Culture & Communication", "Health & Community", "K–12", "Trade School", "Regional", "Cognition.X : Sapient OS", "Cognition.X : Robotics OS", "Cognition.X : Global Health OS", "Cognition.X : Multilateral OS", "Non-Profit Practice", "Cognition.X : Education OS", "Cognition.X : Science OS", "Cognition.X : Corporate OS"}
...
# values outside the five standard bands. Health & Community's two
# adult-route capstones (grade "—") were the only such gap in any
# UNBANDED_FILLED_PACKS member; both were filled in v0.98.0, so this is
# now empty — kept as a dict (not deleted) so a future pack hitting a
# genuine gap has a place to declare it without re-deriving the pattern.
UNBANDED_KNOWN_GAPS = {}

KNOWN_EMPTY_DESCRIPTION_ROWS = 0  # 6,200 before prompt 3 (docs/NEXT_STEPS_2.md #3); 6,089 after Future-Work (v0.90.0, prompt 3 tranche one, 111 rows); 5,867 after Civic & Leadership + Language, Culture & Communication (v0.91.0, prompt 3 tranche two, 222 rows); 5,758 after Health & Community (v0.92.0, prompt 3 tranche three, 109 of its 111 rows — the other 2 use grade "—" and are out of scope); 5,591 after Empathy & Emotional Intelligence (v0.93.0, prompt 3 tranche four, 167 rows, the first "partial_bands" promotion); 5,424 after Community & Relationship Practice (v0.94.0, prompt 3 tranche five, 167 rows); 5,360 after K–12 (v0.95.0, prompt 3 continuation, 64 rows, the first pack reached after extending apply_promotions()'s grade filter from BAND_LEVEL to GRADE_LEVEL); 5,313 after Trade School (v0.96.0, prompt 3 continuation, 47 rows, one row per theme at the single adult-route grade "11–12 · adult"); 5,202 after Regional (v0.97.0, prompt 3 continuation, 111 rows across 83 themes — 4 of them repeated verbatim across 8 rows each, one per named region); 5,200 after Health & Community's last 2 grade-"—" rows (v0.98.0, prompt 3 continuation) — this closes every pack reachable by the unbanded/partial_bands mechanisms; 4,580 after Sapient OS (v0.99.0, prompt 3 final piece, pilot pack, 620 rows across 357 generic-practice-step themes, each reused across many named credentials — apply_promotions() extended to merge multiple promotion files per pack, since Sapient OS already carried a "band-suffix" override promotion from prompt 2); 3,940 after Robotics OS (v0.100.0, prompt 3 final piece, 640 rows across 378 themes, 120 reused verbatim from Sapient OS's dictionary and 258 authored fresh); 3,280 after Global Health OS (v0.101.0, prompt 3 final piece, 660 rows across 349 themes, 112 reused from the shared Sapient/Robotics dictionary and 237 authored fresh); 2,630 after Multilateral OS (v0.102.0, prompt 3 final piece, 650 rows across 352 themes, 156 reused from the shared dictionary and 196 authored fresh); 1,980 after Non-Profit Practice (v0.103.0, prompt 3 final piece, 650 rows across 348 themes, 168 reused from the shared dictionary and 180 authored fresh); 1,320 after Education OS (v0.104.0, prompt 3 final piece, 660 rows across 336 themes, 168 reused from the shared dictionary and 168 authored fresh for its school/district administrative-practice domain); 660 after Science OS (v0.105.0, prompt 3 final piece, 660 rows across 394 themes, 185 reused from the shared dictionary and 209 authored fresh for its scientific research-practice domain); 0 after Corporate OS (v0.106.0, prompt 3 final piece, 660 rows across 415 themes, 211 reused from the shared dictionary and 204 authored fresh for its corporate HR/organizational-practice domain) — this closes prompt 3 entirely, the dataset carries zero empty descriptions; the ratchet only falls
