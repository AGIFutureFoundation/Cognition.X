...
- **`data/promotions/science-os-practice-steps.json`** is a new
  `unbanded` promotion (a second promotion file for a pack that already
  had a `"band-suffix"` override): 394 themes, 660 rows, no suffix,
  `track`/`code` left deferred. 185 of the 394 themes reuse an exact
  sentence already promoted for one of the five prior packs; the other
  209 are new and authored for this pack's scientific research-practice
  domain (research methods, lab operations, publication, mentorship,
  collaboration, tech transfer, sustainability, research integrity). No
  block id, code, track, level, credential, or transfer check changed.
- `tests/test_platform.py`'s `UNBANDED_FILLED_PACKS` gains Science OS;
  `KNOWN_EMPTY_DESCRIPTION_ROWS` falls 1,320 → 660. Dataset-wide
  description coverage rises from 92% to 96%.
- One pack remains — Corporate OS — drawing from a shared generic-action
  dictionary now at 1,605 sentences across seven packs.

## [0.104.0] — 2026-09-18

### Added — prompt 3 final piece: Education OS's 660 empty descriptions filled, shared dictionary now at 1,396 sentences
- **`data/promotions/education-os-practice-steps.json`** is a new
  `unbanded` promotion (Education OS carried no prior promotion file):
  336 themes, 660 rows, no suffix, `track`/`code` left deferred. 168 of
  the 336 themes reuse an exact sentence already promoted for Sapient
  OS, Robotics OS, Global Health OS, Multilateral OS or Non-Profit
  Practice; the other 168 are new and authored for this pack's
  school/district administrative and pedagogical-practice domain
  (curriculum, instruction, assessment, family and community
  engagement, operations, budget, safety, equity, data). No block id,
  code, track, level, credential, or transfer check changed.
- `tests/test_platform.py`'s `UNBANDED_FILLED_PACKS` gains Education
  OS; `KNOWN_EMPTY_DESCRIPTION_ROWS` falls 1,980 → 1,320. Dataset-wide
  description coverage rises from 88% to 92%.
- Two packs remain — Corporate OS, Science OS — drawing from a shared
  generic-action dictionary now at 1,396 sentences across six packs.
...
- **`data/promotions/global-health-os-practice-steps.json`** is a new
  `unbanded` promotion: 349 themes, 660 rows, no suffix, `track`/`code`
  left deferred. 112 of the 349 themes reuse the exact sentence already
  promoted for Sapient OS or Robotics OS; the other 237 are new and
  authored for this pack's global-health-practice domain. No block id,
  code, track, level, credential, or transfer check changed.
- `tests/test_platform.py`'s `UNBANDED_FILLED_PACKS` gains Global
  Health OS; `KNOWN_EMPTY_DESCRIPTION_ROWS` falls 3,940 → 3,280.
  Dataset-wide description coverage rises from 77% to 81%.
- Five packs remain — Corporate OS, Education OS, Science OS,
  Multilateral OS, Non-Profit Practice — drawing from a shared
  generic-action dictionary now at 852 sentences across three packs.
...
- **`data/promotions/housing-tenancy.json`** and
  **`data/promotions/money-benefits-entitlements.json`** are brand-new
  promotion files — neither pack had an existing promotion or
  pack_spec file, and their `track` and `description` were already
  non-empty directly in the raw source, carrying the band-suffix
  content debt from the start rather than arriving through a
  fill-empty or pack-spec path. Each new file declares
  `"override": "band-suffix"`, copies its five tracks and fifty themes
  (`track`, `theme`, `description`) verbatim from
  `data/source/Cognition.X_all_blocks.csv` with the suffix stripped,
  and carries `bands` on every theme: five distinct sentences on the
  band ladder — 500 rows across the two packs; no block id, code,
  credential or transfer check changed.
- The description-override count (`docs/DATA_QUALITY.md`) rises from
  3,500 rows / 7 packs to 4,000 rows / 9 packs — the same counted
  exception used for the seven sector-OS packs (v0.72.0–v0.78.0), now
  extended to the first two community packs. Band-suffix rows
  1,000 → 500. `tests/test_platform.py`'s `BAND_OVERRIDDEN_PACKS` gains
  both packs.
- Only Reentry & Recovery Pathways and Neighbourhood Safety & Civic
  Voice remain for prompt 2, each needing the same new-override-
  promotion mechanism established here.
...
- **`data/pack_specs/civic-leadership-institute-model.json`**,
  **`civic-leadership-legacy-california.json`**, and
  **`civic-leadership-legacy-texas.json`** now carry `bands` on every
  theme: five tracks each, ten themes, five distinct sentences on the
  band ladder — 750 rows across the three packs; no block id, code,
  credential or transfer check changed. All three are pack-spec-based;
  `bands` reaches them through the same direct path used for the other
  pack-spec packs — no override needed. Band-suffix rows 2,500 → 1,750.
...
- **`data/promotions/water-land-climate.json`** and
  **`data/promotions/making-repair-reuse.json`** now carry `bands` on
  every theme: five tracks each, ten themes, five distinct sentences on
  the band ladder — 500 rows across the two packs; no block id, code,
  credential or transfer check changed. Both packs promote from a
  legacy source whose track and description fields are empty, so
  `bands` reaches them through the same fill-empty path as the other
  community packs — no override needed (verified: the override count
  in `docs/DATA_QUALITY.md` stayed at 3,500 rows / 7 packs). Band-suffix
  rows 3,500 → 3,000.
