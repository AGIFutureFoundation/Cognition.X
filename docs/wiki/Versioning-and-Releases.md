...
| v0.93.0 | Prompt 3 tranche four: a new `"partial_bands": true` promotion mechanism for packs where a theme's rows span only some of the five bands, and Empathy & Emotional Intelligence's 167 empty descriptions filled through it (55 themes, each theme's own actual 3–4-grade span, no suffix, `track`/`code` deferred like `unbanded`); empty-description rows 5,758 → 5,591 |
| v0.94.0 | Prompt 3 tranche five: Community & Relationship Practice's 167 empty descriptions filled through the `partial_bands` mechanism (55 themes, same shape as Empathy); closes every foundation pack reachable by `unbanded`/`partial_bands` — empty-description rows 5,591 → 5,424 |
| v0.95.0 | Prompt 3 continuation: `apply_promotions()`'s grade filter extended from `BAND_LEVEL` to `GRADE_LEVEL` to reach single-grade and adult-route rows, and K–12's 64 empty descriptions filled through the `unbanded` mechanism (13 single grades K–12, one theme per row) — empty-description rows 5,424 → 5,360 |
| v0.96.0 | Prompt 3 continuation: Trade School's 47 empty descriptions filled through the `unbanded` mechanism (12 trade families, one theme per row at the single adult-route grade `11–12 · adult`) — empty-description rows 5,360 → 5,313 |
| **v0.97.0** | Prompt 3 continuation: Regional's 111 empty descriptions filled through `unbanded` (8 regions × 9 themes, a 4-theme cross-region track, a 7-theme industry-cluster track), and the `unbanded` distinctness check scoped to themes rather than rows — empty-description rows 5,313 → 5,202 |
| v0.92.0 | Prompt 3 tranche three: Health & Community's 109 (of 111) empty descriptions filled through the same `"unbanded": true` mechanism; two adult-route, grade-`—` rows stay out of scope. Found that Empathy & Emotional Intelligence and Community & Relationship Practice do not fit `unbanded` (themes recur across 3–4 grades irregularly) — a further mechanism, scoped not built. Empty-description rows 5,867 → 5,758; dataset-wide description coverage 66% → 67% |
| v0.91.0 | Prompt 3 tranche two: Civic & Leadership and Language, Culture & Communication's 222 empty descriptions filled through the same `"unbanded": true` mechanism as v0.90.0 — no block id changed; empty-description rows 6,089 → 5,867; dataset-wide description coverage 65% → 66% |
| v0.90.0 | Prompt 3 tranche one: Future-Work's 111 empty descriptions filled via a new `"unbanded": true` promotion mechanism — for a pack where each theme names exactly one row, not five across the band ladder, filling in one complete sentence with no suffix (and skipping `track`/`code`, which would otherwise fail the tracked-shape check); no block id changed; empty-description rows 6,200 → 6,089; dataset-wide description coverage 64% → 65% |
| v0.89.0 | Prompt 2 tranche eleven, the last: Reentry & Recovery Pathways and Neighbourhood, Safety & Civic Voice carry `bands` on every theme through the same new `override: band-suffix` promotion mechanism as v0.88.0 — 500 rows across two packs, no block id changed; description-override count rises to 4,500 rows / 11 packs; band-suffix rows 500 → 0. Prompt 2 of `docs/NEXT_STEPS_2.md` is closed — every band-suffix row in the dataset now carries an authored per-band sentence |
| v0.88.0 | Prompt 2 tranche ten: Housing & Tenancy and Money, Benefits & Entitlements — the first two community packs whose source descriptions were already non-empty — carry `bands` on every theme through a brand-new `override: band-suffix` promotion, the same counted exception used for the sector-OS packs — 500 rows across two packs, no block id changed; the description-override count rises to 4,000 rows / 9 packs; band-suffix rows 1,000 → 500 |
